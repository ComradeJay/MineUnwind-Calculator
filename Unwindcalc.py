...
if dim=="City": # 𝟭𝘀𝘁 𝗥𝗲𝗮𝗹𝗺
     ...
     if block=='Stone - Iron Mix':
          ...
          if ((ID % 64)==0):
               ...
               st.header(f"You need {int(ID / 64):,d} Stacks of Iron - Diamond Mix and {ID%64} T3 Iron and Diamond")
          
          
          if ((DE % 64)==0):
               st.header(f"You need {int(DE / 64):,d} Stacks of Diamond - Emerald Mix")
          else:
               st.header(f"You need {int(DE/64):,d} Stacks and {DE%64} T3 Diamond and Emerald")
          # if ((Emerald % 64)==0):
